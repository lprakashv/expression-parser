...
import com.lprakashv.models.MultiTermExpression.{
  AddSeqExpression,
  DivSeqExpression,
  ExponentExpression,
  MulSeqExpression
...
object Parsers {
  ...
  def parentheses[_: P]: P[Parentheses] =
    P(
      ws ~
        "(".rep(exactly = 1) ~ ws ~
        additions ~
  ...
  def singleTerm[_: P]: P[SingleTermExpression] =
    P(parentheses | function | number)

  // clubbed operations can be considered a single unit (`2 * 5 * 23`, `2 ^ 1 ^ 4` etc)
  def multiTermSeqExpressionGeneric[_: P](
    binaryOperators: Set[BinaryOperator],
  ...
  def multiTermSeqExpression[_: P](
    binaryOperator: BinaryOperator,
    overExpression: => P[Expression]
  ): P[(Expression, Seq[Expression])] =
    multiTermSeqExpressionGeneric(Set(binaryOperator), overExpression).map {
      case (first, operationsWithOperator) =>
        (first, operationsWithOperator.map(_._2))
    }

  // multiTermSeqExpression with '^' as operator
  def exponent[_: P]: P[ExponentExpression] =
    multiTermSeqExpression(BinaryOperator.Pow, singleTerm).map {
      case (first: Expression, subsequent: Seq[Expression]) =>
        ExponentExpression(first, subsequent.toList)
    }

  // multiTermSeqExpression with '/' as operator
  def division[_: P]: P[DivSeqExpression] =
    multiTermSeqExpression(BinaryOperator.Divide, exponent).map {
      case (first: Expression, subsequent: Seq[Expression]) =>
        DivSeqExpression(first, subsequent.toList)
    }

  // multiTermSeqExpression with '*' as operator
  def multiplication[_: P]: P[MulSeqExpression] =
    multiTermSeqExpression(BinaryOperator.Multiply, division).map {
      case (first: Expression, subsequent: Seq[Expression]) =>
        MulSeqExpression(first, subsequent.toList)
    }

  // multiTermSeqExpressionGeneric with '+' and '-' as operators
  def additions[_: P]: P[AddSeqExpression] =
    multiTermSeqExpressionGeneric(
      Set(BinaryOperator.Plus, BinaryOperator.Minus),
      multiplication
    ).map {
      case (first: Expression, subsequent: Seq[(String, Expression)]) =>
        val onlyAdditionSubsequent: Seq[Expression] = subsequent.map {
          case ("-", expr) => MulSeqExpression(Number(-1), List(expr))
          case ("+", expr) => expr
          case (o, expr) =>
            throw new RuntimeException(s"Invalid operation $o on $expr")
        }

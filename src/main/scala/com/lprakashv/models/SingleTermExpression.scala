...
object SingleTermExpression {
  ...
  case class Fn(name: String, args: Seq[Expression])
      extends SingleTermExpression {
    override def eval: Value = {
      val evaluatedArgs = args.zip(args.map(_.eval))

      val errorOpt = evaluatedArgs.collectFirst {
        case (e, ErrorValue(msg)) =>
          ErrorValue(s"Error: [$msg] in expression: [ $e ]")
      }
      ...
      errorOpt.getOrElse(FunctionStore.getFunction(name) match {
